 ...
 * @return        0=正常
 */
int read_vtk(const char *filename, vtk_t &vtk_info)
{
    FILE *fp;
    char line[256];
    int n;
    ...
    while( fgets( line, 256, fp ) != NULL ){
        ...
        if (strcasecmp(split_list[0], "CELLS") == 0) {
            ...
            vtk_info.vtk_cells = (vtk_cell_t*)malloc(sizeof(vtk_cell_t)*num_cells);
...
int main(int argc, char *argv[]) {
    ...
    if (myrank == 0) {
        ...
        for (n=0; n<vtk_info.num_points; n++) {
            ...
            UdmSize_t node_id = grid->insertGridCoordinates(x, y, z);
        }
        ...
        for (n=0; n<vtk_info.num_cells; n++) {
            ...
            elements->insertCellConnectivity(cell_type, elem_nodes);
            free(elem_nodes);
        }
