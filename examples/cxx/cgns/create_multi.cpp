...
int main(int argc, char *argv[]) {
    ...
    for (k=0; k<grid_size[2]; k++) {
        for (j=0; j<grid_size[1]; j++) {
            for (i=0; i<grid_size[0]; i++) {
                ...
                if (myrank > 0 && i == 0) {
                    grid->insertRankConnectivity(node_id, myrank-1, node_id+grid_size[0]-1);
                }
    ...
    // 分割後ファイル出力 : step = 0
    model->initializeTimeSlice();
    config->setOutputPath(output_partition);
    ret = model->writeModel(0, 0.0);
    if (ret != UDM_OK) {
        cout << "Error : can not write model" << endl;
        delete model;
